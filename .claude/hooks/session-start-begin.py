...
**この hook は Claude に知らせを渡すだけで、Claude は人の最初の一言を待つ**（day-8 に分かった）。
何も打たずに始めるのは `start.sh` の役目で、`lead` を起動したあと最初の一言として `/始め` を送る（#194・#205）。
この hook は、`claude` を手で打ったとき（`start.sh` を通らないとき）の保険として残す。
