...
> **この README は「収録1本」の前提で書いてある。**
> 番組は複数コーナー入りの30分に決まったが、入力を「wav 複数本」にする対応はまだ入っていない。
> 設計は [#85](https://github.com/Bohemian1506/okuradi/issues/85) にまとめてあり、
> [#53](https://github.com/Bohemian1506/okuradi/issues/53)（通しリハーサル）を通してから着手する。
...
**GUI が扱うのは7工程**（音源・下見・整音・ミックス（曲を重ねる）・文字起こし・メタデータ・動画）。
`cut`（途中のカット）と `upload`（限定公開）はコマンドだけ。アップロードは手で貼るため。

**`cut` は GUI に出すと決めた**（2026-09-22・`docs/features.md` の「決定3を取り下げた理由」）。
タイムライン上で区間を選び、**カットかエコーかを選ぶ**形にする（#79 / #85）。**まだ実装していない。**
...
└── ep01/
    ├── config.yml        # この回の設定。人が手で書く
    ├── timeline.yml      # クリップの配置と編集点。画面が書き換える（無い回もある）
    ├── 00_raw/           # 収録ファイルを置く（wav / m4a / OBSの録画）
    ├── 01_cut/           # カット後
    ├── 01_clean/         # 整音後
    ├── 01_mix/           # timeline.yml の BGM・SE を重ねた後（無い回は clean.wav と同じ音）
    ├── 02_text/          # 文字起こし
...
> **移す先が決まっている。** エコー区間は `timeline.yml` の `edits`（カットと同じ並び）へ移し、
> 時刻も `trimmed.wav` 基準から**生音基準**に変える（2026-09-22・#79 / #153）。**まだ移していない。**
...
- BGM: `mix` 工程で実装済み（#85）。`timeline.yml` の `bgm` の行に音源を並べる（`config.yml` の
  `bgm` ではない）
