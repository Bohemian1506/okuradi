...
| #96 | `series_rules` を30分番組のコーナー名に揃える | **番組側が立てた Issue。ラベルが無い。** 中身は決定済みで、**いま実装できる**（#85 や #87 に依存しない） |
| #79 #82 #85 #87〜#93 | 30分番組化の設計 | 全部「段取り」。着手していない。地図は #85 |
| #97 #98 #99 | 倍率まわり（測定・時刻を保つ・操作を作る） | #97 は測定済み。#98 #99 は**前提が立っていないので着手しない** |

**#96 は `ep01/config.yml` を触る。** その回のファイルには**ユーザーの未コミットの変更（エコー区間）がある**ので、
混ぜないよう気をつける。テスト2か所（`tests/test_chat.py` / `tests/test_episodes.py`）の追随も要る。

**#86 は閉じた**（下記）。塞がれていた #87〜#93 は前提が外れたが、**着手はしない**。
...
- **番組側（r-hoso）の出力は、すべて GitHub から来る。** メッセージの返信ではなく、
  **#86 へのコメント**や**新しい Issue（#96）**として現れる。
  返事を待つあいだ「返事はまだ来ていない」と3回書き、#96 は起票から半日気づかなかった。
