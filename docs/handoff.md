...
- **番組のリポジトリを分けた。** 番組の仕様・構成・台本・収録メモは [r-hoso](https://github.com/Bohemian1506/r-hoso)（非公開、`~/workspace/r-hoso`）。okuradi はツールだけ
...
| #79 #82 #85〜#93 | 30分番組化の設計（mix / タイムライン編集 / 切り抜き） | **11件。全部「段取り」で、着手していない**。地図は #85 |

**#86 が他を止めている。** SE をレーンで分けてよいかを決めるには「SE を何個使うか」が要り、それは**番組側（r-hoso）の話**。まだ決まっていない。

### 次にやること
1. **r-hoso で番組の構成を詰める**（`./start.sh --with-hoso` か、F12 の「okuradi + アール放送局」）
   - とくに **SE と BGM をどこで何個使うか**。ここが決まらないと #86 が決められない
   - r-hoso は**別の `CLAUDE.md`** を読む（番組向けのルール）
...
4. **設計の11件は着手しない。** #53 を通してから。30分の形に手を入れてから壊れると、切り分けができなくなる
...
- **Issue を書く前に実機で測る。** #82 で、既存のやり方を流用する案が使えないと分かった（尺が 0.04秒 縮む）。#85 でも、ライブラリを動かしたら #82 の論点の半分が消えた
- **「どう作るか」より先に「何を作るか」。** 番組の形が決まる前に設計を詰めると、論点の前提が浮く
- **一時領域（`/tmp/claude-1000/`）は消えることがある。** CDP の確認ツールを作り直した
