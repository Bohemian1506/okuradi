...
| #86 | 1レーン=音源1つ をどう受け止めるか | **争点は消えた。ユーザーの確定待ち**（下記） |
...
番組側は #86 のコメントで **「案E' で問題ありません」「marker は使わなくて大丈夫です」** と答えている
（理由: SE の位置は聴いて合わせたい）。

**その後、争点そのものが消えた。** SE は音源のまま置き（案E'）、クリップに CSS で最低幅
（`min-width: 14px`）を与えると、**「鳴る」と「見える」が両立する**。marker を使う理由が無くなった。
測定は #97 のコメント、結論は #86 のコメント。

**残っているのはユーザーの確定だけ。** 覆す材料は今のところ無い。
...
- **返事は、送った経路とは別の所に来ることがある。** r-hoso へメッセージを送った返事は #86 のコメントで返ってきたのに、
  メッセージの返信ばかり待って「返事はまだ来ていない」と3回書いた。**送ったら、返ってくる経路（Issue）を見直す**
