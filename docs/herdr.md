...
3. `main` タブで、メインの Claude を `lead` という名前で起動する。すでに動いていれば起動しない。
   **起動して `lead` の名前が付いたあとで、最初の一言として `/始め` を送る**（#194。何も打たずに突き合わせが始まる形）。
   続きから（`-c`）は、`/作業終了` の印があるときだけ送る。
   **起動の引数では渡さない**（#205）。渡すと Claude がすぐ作業を始めて入力待ちにならず、Herdr が名前を付けないまま失敗扱いにする
...
- その日の作業を終える: Claude で `/作業終了`。区切っていなければ先に区切り、作業のペインを閉じて印を置く。
  **印があると、次に `start.sh -c`（続きから）で開いたときも `/始め` が走る**（#183・#194）。
...
- **GUIログで何か動いていると、`close` は閉じない。** 裏の工程が取り残されるため。サーバーを止めてから閉じる
